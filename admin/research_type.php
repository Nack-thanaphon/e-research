...
{	
	...
	$sql = "select `et_name` from `ers_research_type` where (`et_name`='$s_et_name') ";
	...
	if($num_rows > 0) {
		...
		<Script language="javascript">
			alert("ประเภทของเงินอุดหนุนงานวิจัย : <?= $s_et_name;?> มีอยู่ในระบบ ไม่สามารถบันทึกซ้ำได้");
			window.location="research_type.php?iRegister=1";
		</script>
		<?php
		die();
	}

	$sql = "select * from `ers_research_type` where (`id`='".$c_id."')";
	$dbquery = $mysqli->query($link,$sql) or die("Can't send query !!");
	$num_rows = $dbquery->num_rows;
	$dbquery->close();
	if($num_rows > 0) {
		$sql = "update `ers_research_type` set `et_name`='$s_et_name',`update_date`=now(),`update_user`='$admin' where (`id`='".$c_id."')";
		$dbquery = $mysqli->query($link,$sql) or die("ไม่สามารถบันทึกข้อมูลได้ !1");
	}else {
		if($s_et_name != ''){
			$sql = "insert into `ers_research_type` (`et_name`,`update_date`,`update_user`) values ('$s_et_name',now(),'$admin')";
...
function c_check(){
	...
	{
		alert("'ชื่อ ประเภทของเงินอุดหนุนงานวิจัย' จำเป็นต้องมีข้อมูล");
...
<div class="container">
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 clearmp">
		   <div class="col-sm-12 col-xs-12" style="border-bottom:1px solid #003300;font-size:18px;color:#003300;"><span class="glyphicon glyphicon-plus"></span>&nbsp;ประเภทของเงินอุดหนุนงานวิจัย</div>
	...
	<div class="row">
		<div class="col-lg-6 col-md- 8 col-sm-10 col-xs-12 col-lg-offset-3 col-md-offset-2 col-sm-offset-1" style="padding-top:20px;padding-bottom:20px;">
		  ...
		  <form name="form1"  method="post" action="research_type.php" onSubmit="return c_check();" role="form">
			  <div class="col-lg-12 col-md-12 col-sm-12 text-center">
				<div class="col-lg-5 col-md-5 col-sm-5 col-xs-5 text-right">ชื่อประเภทของเงินอุดหนุนงานวิจัย&nbsp;:&nbsp;</div>
				<div class="col-lg-7 col-md-7 col-sm-7 col-xs-7"><input type="text" name="s_et_name" id="s_et_name" maxlength="255" class="form-control input_width"  value="<?= $c_et_name;?>"></div>
			  </div>
			  <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" >&nbsp;</div>
	...
	<div class="row">
		...
		<form name="form2" method="post" action="research_type.php#top_page" onSubmit="return c_check2();" role="form">
		  <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
			 <div style="text-align:center;">
				<span class="sbreak2">ค้นหา : <input type="text" name="c_code_1" id="c_code_1" class="searchbox-control" maxlength="30" placeholder="ข้อความที่ต้องการค้นหา">&nbsp;<input type="submit" name="Submit" id="Submit" value="ค้นหา" class="btn btn-success" style="width:70px;padding-left:3px;">&nbsp;<input type="button" name="Clear" id="Clear" value="ยกเลิก" class="btn btn-info" style="width:70px;margin-top:0;" onclick="window.location='research_type.php?iRegister=1';"></span>
			  </div>
		  </div>
		  </form>
		  ...
		  <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center" style="background-color:#<?php echo __EC_BGSHOW__;?>;color:#<?php echo __EC_FONTSHOW__;?>;border-radius: 5px 5px 0px 0px;"><h4>แสดงข้อมูล ประเภทของเงินอุดหนุนงานวิจัย</h4><a name="top_page"></a></div>
			...
			<table class="table table-striped">
              <thead>
                <tr style="background-color:#f7f7f7">
                  <th>&nbsp;</th>
				  <th>&nbsp;</th>
                  <th style="text-align:center;"><a href="research_type.php?sd=1&sh_order=<?= $sh_order;?>#top_page" target="_parent">ID <?= $a1sort;?></a></th>
                  <th><a href="research_type.php?sd=2&sh_order=<?= $sh_order;?>#top_page" target="_parent">ชื่อประเภทของเงินอุดหนุนงานวิจัย <?= $a2sort;?></a></th>
                </tr>
              </thead>
              <tbody>
				...
				$sql = "select * From `ers_research_type` where 1 ";
				...
				if($totalRows!="0"){
					...
					?>
						...
						<?php
						echo "<a href='research_type.php?c_id=$c_id' style='color:green;font-size:16px;' title='แก้ไข'><span class='glyphicon glyphicon-edit'></span>&nbsp;<span style='font-size:14px;'>แก้ไข</span></a>";
						?>
						</td>
						<td style="text-align:center;width:100px;min-width:100px;">
						<?php
						echo "<a href='del_data.php?c_id=$c_id&chk_p=9&code_1=$code_1' style='color:red;font-size:16px;' title='ลบ'><span class='glyphicon glyphicon-trash'></span>&nbsp;<span style='font-size:14px;'>ลบ</span></a>";
						?>
						</td>
