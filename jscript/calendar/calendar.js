...
// $Id: calendar.js,v 1.1 2005/02/04 04:49:49 jeep Exp $
...
Calendar.dayMouseOver = function(ev) {
	...
	if (el.ttip) {
		if (el.ttip.substr(0, 1) == "_") {
			...
			var tmp1 = date.print(el.calendar.ttDateFormat);
			var tmp2 = tmp1.substr(tmp1.length - 4);
			tmp1 = tmp1.substr(0, tmp1.length - 4) + (parseInt(tmp2));
			//tmp1 = tmp1.substr(0, tmp1.length - 4) + (parseInt(tmp2)+543);
			el.ttip = tmp1 + el.ttip.substr(1);
		}
...
function Validate(inform) {
	if (inform.Start_Date.value == "")
	{
		alert("Please Input Start Date");
		return false;
	}else{
		if (inform.End_Date.value == "")
		{
...
function OnSubmit(inform) {
	if (Validate(inform) == true)
	{
		inform.submit();
	}
}
