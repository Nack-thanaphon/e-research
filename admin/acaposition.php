...
{	
	...
	$s_ea_name = $_POST["s_ea_name"];

	$sql = "select `ea_name` from `ers_academic_position` where (`ea_name`='$s_ea_name') ";
	...
	$sql = "select * from `ers_academic_position` where (`id`='".$c_id."')";
	$dbquery = $mysqli->query($sql) or die("Can't send query !!");
	$num_rows = $dbquery->num_rows;
	$dbquery->close();
	if($num_rows > 0) {
		$sql = "update `ers_academic_position` set `ea_name`='$s_ea_name',`update_date`=now(),`update_user`='$admin' where (`id`='".$c_id."')";
		$dbquery = $mysqli->query($sql) or die("ไม่สามารถบันทึกข้อมูลได้ !1");
	}else {
		if($s_ea_name != ''){
			$sql = "insert into `ers_academic_position` (`ea_name`,`update_date`,`update_user`) values ('$s_ea_name',now(),'$admin')";
...
<SCRIPT LANGUAGE="JavaScript">
function c_check(){
	if(document.getElementById('s_ea_name').value == "")
	{
		alert("'ชื่อ ตำแหน่งทางวิชาการ' จำเป็นต้องมีข้อมูล");
		document.getElementById('s_ea_name').focus();
		return false;
	}
}
function c_check2(){
...
<div class="container">
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 clearmp">
		   <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" style="border-bottom:1px solid #00cc00;font-size:18px;color:#00cc00;"><span class="glyphicon glyphicon-plus"></span>&nbsp; ตำแหน่งทางวิชาการ</div>
	...
	<div class="row">
		...
		<hr align="center" width="100%" noshade size="1">
		  <form name="form2" method="post" action="acaposition.php#top_page" onSubmit="return c_check2();" role="form">
		  <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
			  <div style="text-align:center;">
				<span class="sbreak2">ค้นหา : <input type="text" name="c_code_1" id="c_code_1" class="searchbox-control" maxlength="30" placeholder="ข้อความที่ต้องการค้นหา">&nbsp;<input type="submit" name="Submit" id="Submit" value="ค้นหา" class="btn btn-success" style="width:70px;padding-left:3px;">&nbsp;<input type="button" name="Clear" id="Clear" value="ยกเลิก" class="btn btn-info" style="width:70px;margin-top:0;" onclick="window.location='acaposition.php?iRegister=1';"></span>
			  </div>
		  </div>
		  </form>
		  ...
		  <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center" style="background-color:#<?php echo __EC_BGSHOW__;?>;color:#<?php echo __EC_FONTSHOW__;?>;border-radius: 5px 5px 0px 0px;"><h4>แสดงข้อมูล ตำแหน่งทางวิชาการ</h4><a name="top_page"></a></div>
		  ...
		  <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			...
			$sql = "select * From `ers_academic_position` where 1 ";
			...
			if(isset($_SESSION["u_code_1"]))
			{
				 ...
				 $sql .= "and ((`id` = '$u_code_1') or (`ea_name` like '%$u_code_1%')) ";
			}
			if(isset($_GET["sd"])){$sd = trim($_GET["sd"]);} else {	$sd = 0;}
			switch ($sd) {
				case '1': $sql .= "Order by `id` "; break;
				case '2': $sql .= "Order by `ea_name` "; break;
