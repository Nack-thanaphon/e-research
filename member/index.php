...
if (!isset($_SESSION["username"]) || !isset($_SESSION["membername"]) || !isset($_SESSION["memberid"]) || empty($_SESSION["username"])) {
	echo "<Script language=\"javascript\">window.location=\"login.php?dcid=$dcid\"</script>";
	exit();
}
...
if (isset($dcid)) {
	if (!empty($dcid)) {
		$sql = "select * from `ers_document` where (`id`='$dcid')";
		$dbquery = $mysqli->query($link, $sql);
		$tRows = $dbquery->num_rows;
		if ($tRows > 0) {
			$row = $dbquery->fetch_assoc();
...
if ($chk_edit == "1") {
	...
	$sql = "select * from `ers_member_request` where (`id`='" . $c_id . "')";
	$dbquery = $mysqli->query($link, $sql) or die("Can't send query !!");
	$num_rows = $dbquery->num_rows;
	$dbquery->close();
	if ($num_rows > 0) {
		$sql = "update `ers_member_request` set `er_request_text`='$s_er_request_text',`er_request_date`=now(),`er_request_cancel`='0',`update_date`=now(),`update_user`='$username' where (`id`='" . $c_id . "') and (`er_answer`='0') and (`member_id`='$memberid')";
		$dbquery = $mysqli->query($link, $sql) or die("ไม่สามารถบันทึกข้อมูลได้ !1");
	} else {
		if (!empty($s_er_request_text) && ($dcid > 0)) {
			$sql = "insert into `ers_member_request` (`member_id`,`document_id`,`er_request_text`,`er_request_date`,`update_date`,`update_user`) values ('$memberid','$dcid','$s_er_request_text',now(),now(),'$username')";
	...
	$s_con_message = "ขอเอกสารผลงานวิจัย : ".$c_ed_name_th."<br>ชื่อ : ".$c_name."<br>เบอร์โทร : ".$c_phone."<br>อีเมล์ : ".$c_email."<br>หน่วยงานที่สังกัด : ".$c_institution." ".$c_institution_name."<br>ที่อยู่ : ".$c_address;
	$s_con_name = $c_name;
...
$mysqli->query($link, "update `ers_document` set `ed_counter`=`ed_counter`+1 where `id` ='" . $dcid . "'");
?>
<!DOCTYPE html>
<html lang="th">
...
<body role="document" style="background: url('../images/<?php if (defined('__EC_PICHOME__')) {
															echo __EC_PICHOME__;
														} ?>') no-repeat;background-attachment:fixed; background-size: 60% auto;height:auto;width:auto;background-position: center;">
	<a name="detailtop"></a>
	...
	<div class="container-fluid" style="width:100vw; height:100vh;-moz-opacity: 0.97;-khtml-opacity: 0.97;opacity: 0.97; background-color: #ffffff;">
		...
		<div class="row">
			...
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-left">
				<div class="h1member">ยีนดีต้อนรับคุณ <font color="#000099"><?= $c_firstname, " " . $c_lastname; ?></font>
				</div>
				<div class="h2member">ชื่อ Login&nbsp;:&nbsp;<font color="#000099"><?= $c_username; ?></font>&nbsp;<?php if (isset($_SESSION["memberpass"])) {
																														echo ",รหัสผ่าน : <font color='#000099'>" . $_SESSION["memberpass"] . "</font>";
																													} ?></div>
				...
				<?php if (!empty($dcid)) { ?>
					<form name="form1" method="post" action="index.php" onSubmit="return c_check();" role="form">
						...
						<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
							...
							<input type="hidden" name="dcid" value="<?php if (isset($dcid)) {
																		echo $dcid;
																	} else {
																		echo '';
																	} ?>">
		...
		<div class="row bgw">
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center" style="background-color:#<?php echo __EC_BGSHOW__; ?>;color:#<?php echo __EC_FONTSHOW__; ?>;">
				<h4 class="sub-header">แสดงข้อมูล ข้อความร้องขอเอกสารงานวิจัย (งานวิจัยในระบบ)</h4>
			</div>
			<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
					...
					<table class="table table-striped table-bordered sticky-header" id="table-1">
						<thead>
							<tr id="ontop1" style="background-color:#e5e5e5">
								...
								<!--<th style="text-align:center;" id="ontop11">&nbsp;ตอบโดย&nbsp;</th>-->
							</tr>
						</thead>
						<tbody class="bgw">
							<?php
							...
							switch ($sd) {
								case '1':
									...
									break;
								case '2':
									$sql .= "Order by `er_request_text` ";
									break;
								case '3':
									$sql .= "Order by `er_request_date` ";
									break;
								case '4':
									$sql .= "Order by `er_answer` ";
									break;
								case '5':
									$sql .= "Order by `er_answer_text` ";
									break;
								case '6':
									$sql .= "Order by `er_answer_date` ";
									break;
								case '7':
									$sql .= "Order by `er_answer_name` ";
									break;
								case '8':
							...
							if ($totalRows != "0") {
								...
								while ($result = $res->fetch_assoc()) {
									...
									if (!empty($result["er_answer_expire"])) {
										$expire_date = $result["er_answer_expire"];
									...
									$c_er_answer_name = $result["er_answer_name"];
									...
									<tr>
										...
										<td style="text-align:center;">
											<?php if (($c_er_answer == 1) && ($chk_expire == 1)) { ?>
												<a href="javascript:void(0)" onclick="show_detail('<?= $c_id; ?>', '1')" style="color:#000000;">
													<div style="background-color:#f8dc67;padding:4px 1px 2px 1px;border-radius:5px;"><span class="glyphicon glyphicon-download" style="color:#006633;font-size:14px;"></span>&nbsp;ดาวน์โหลด</div>
												</a>
											<?php } ?>
										</td>
										<td style="text-align:left;">&nbsp;
										...
										<!--<td style="text-align:center;"><? php //=$c_er_answer_name;
																			?></td>-->
									</tr>
							<?php
								} //while
								...
								echo "<tr class='text-center' style='background-color:#ffffff'><td colspan='11'><br><span style='color:#ff0000;font-size:16px;'>..ไม่พบข้อมูล..</span></td></tr>";
							}
							?>
						</tbody>
					</table>
...
<script>
	function confirmCancel(span_id, id_order, filename, content_id) {
		if (confirm("ยืนยันการยกเลิกการร้องขอเอกสาร " + filename)) {
			ajax_loadContent(span_id, 'can_req.php', id_order, 'member_request', content_id);
