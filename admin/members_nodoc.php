...
$sql = "select m.*,d.*,m.id as req_id FROM `ers_member_request_nodoc` m JOIN `ers_member` d ON m.member_id=d.id where (`er_request_cancel`='0') and (`er_answer`='1') and (d.id > '0') ";
if(isset($_SESSION["u_code_1"]) and !empty($_SESSION["u_code_1"]))
{
	  ...
	  if(strpos( $u_code_1," ")>=1){
		  ...
		  $sql .= " or (`er_request_text` like '%$u_search%') or (`er_answer_text` like '%$u_search%') or (`em_username` like '%$u_search%') or (`em_title` like '%$u_search%') or (`em_firstname` like '%$u_search%') or (`em_lastname` like '%$u_search%') or (`em_institution` like '%$u_search%') or (`em_institution_other` like '%$u_search%') or (`em_phone` like '%$u_search%') or (`em_email` like '%$u_search%') or (`em_address` like '%$u_search%') ";
		  for($i = 1, $size = count($u_code_array); $i < $size; ++$i) {
			   $u_search = $u_code_array[$i];
			   $sql .= ") and ((`er_request_text` like '%$u_search%') or (`er_answer_text` like '%$u_search%') or (`em_username` like '%$u_search%') or (`em_title` like '%$u_search%') or (`em_firstname` like '%$u_search%') or (`em_lastname` like '%$u_search%') or (`em_institution` like '%$u_search%') or (`em_institution_other` like '%$u_search%') or (`em_phone` like '%$u_search%') or (`em_email` like '%$u_search%') or (`em_address` like '%$u_search%') ";
		  }
	  } else {
		  $sql .= " or (`er_request_text` like '%$u_code_1%') or (`er_answer_text` like '%$u_code_1%') or (`em_username` like '%$u_code_1%') or (`em_title` like '%$u_code_1%') or (`em_firstname` like '%$u_code_1%') or (`em_lastname` like '%$u_code_1%') or (`em_institution` like '%$u_code_1%') or (`em_institution_other` like '%$u_code_1%') or (`em_phone` like '%$u_code_1%') or (`em_email` like '%$u_code_1%') or (`em_address` like '%$u_code_1%') ";
	  }
	  $sql .= ") ";
}
$dbquery = $mysqli->query($sql) or die("Can't send query!");
$tRows_y = $dbquery->num_rows;
$sql = "select m.*,d.*,m.id as req_id FROM `ers_member_request_nodoc` m JOIN `ers_member` d ON m.member_id=d.id where (`er_request_cancel`='0') and (`er_answer`='0') and (d.id > '0') ";
if(isset($_SESSION["u_code_1"]) and !empty($_SESSION["u_code_1"]))
{
	  $u_code_1 = $_SESSION["u_code_1"];
	  $u_code_1 = trim($u_code_1);
...
$sql = "select m.*,d.*,m.id as req_id FROM `ers_member_request_nodoc` m JOIN `ers_member` d ON m.member_id=d.id where (`er_request_cancel`='1')  and (d.id > '0') ";
if(isset($_SESSION["u_code_1"]) and !empty($_SESSION["u_code_1"]))
{
	  $u_code_1 = $_SESSION["u_code_1"];
	  $u_code_1 = trim($u_code_1);
...
<div class="container-fluid">
	...
	<div class="row" style="padding-top:50px;padding-bottom:10px;">			  
		<div id="hSelect" class="hSelect">
			...
			<form name="form2" method="post" action="members_nodoc.php" role="form">
			    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
					  ...
					  <input type="submit" name="Submit" id="Submit" value="ค้นหา" class="btn btn-success" style="width:70px; margin:2px;">&nbsp;<input type="button" name="Clear" id="Clear" value="ยกเลิก" class="btn btn-info" style="width:70px;margin-top:0;" onclick="window.location='members_nodoc.php?iRegister=1';"></span>
			    </div>
			</form>
	...
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			...
			<span class="glyphicon glyphicon-flag" style="color:#00cc00;"></span>&nbsp;ตอบกลับแล้ว : <font style='color:#00cc00;'><?= number_format($tRows_y);?></font>&nbsp;&nbsp;
	...
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center" style="background-color:#<?php echo __EC_BGSHOW__;?>;color:#<?php echo __EC_FONTSHOW__;?>;"><h4 class="sub-header">ตอบกลับการร้องขอผลงานวิจัย(นอกระบบ)</h4></div>
		 <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 text-center">
			...
			<table class="table table-striped table-bordered sticky-header" id="table-1" style="background: url('../images/<?php if(defined('__EC_PICHOME__')){echo __EC_PICHOME__;}?>') repeat-y; background-attachment:fixed; background-size:contain;height:100%;width:100%;">
				<thead>
					<tr id="ontop1" style="background-color:#e5e5e5">
						  <th id="ontop2">&nbsp;</th>
						  <th style="vertical-align:middle;text-align:center;" id="ontop3"><a href="members_nodoc.php?sd=1&sh_order=<?= $sh_order;?>" target="_parent" style="white-space: nowrap;">ID <?= $a1sort;?></a></th>
						  <th style="vertical-align:middle;text-align:center;"  id="ontop4"><a href="members_nodoc.php?sd=2&sh_order=<?= $sh_order;?>" target="_parent" style="white-space: nowrap;">ชื่อ-นามสกุล <?= $a2sort;?></a></th>
						  <th style="vertical-align:middle;text-align:center;"  id="ontop5"><a href="members_nodoc.php?sd=3&sh_order=<?= $sh_order;?>" target="_parent" style="white-space: nowrap;">ชื่อ login <?= $a3sort;?></a></th>
						  ...
						  <th style="vertical-align:middle;text-align:center;" id="ontop10">&nbsp;<a href="members_nodoc.php?sd=4&sh_order=<?= $sh_order;?>" target="_parent" style="white-space: nowrap;">วันที่ร้องขอ <?= $a3sort;?></a>&nbsp;</th>
						  <th style="vertical-align:middle;text-align:center;" id="ontop11">&nbsp;<a href="members_nodoc.php?sd=5&sh_order=<?= $sh_order;?>" target="_parent" >ผลการดำเนินการ <?= $a4sort;?></a>&nbsp;</th>
				...
				<tbody class="bgw">
				<?php
				$sql = "SELECT m . * , d . *,m.id as req_id FROM `ers_member_request_nodoc` m JOIN `ers_member` d ON m.member_id = d.id WHERE (d.id > 0) ";
				if(isset($_SESSION["u_code_1"]) and !empty($_SESSION["u_code_1"]))
				{
					  $u_code_1 = $_SESSION["u_code_1"];
					  $u_code_1 = trim($u_code_1);
				...
				if($totalRows > 0){
					...
					echo "<tr class='text-center' style='background-color:#ffffff'><td colspan='11'><br><span style='color:#ff0000;font-size:18px;'>..ไม่พบข้อมูล..</span></td></tr>";
				}
				?>					
				</tbody>
			</table>
...
function confirmDelete(span_id,id_member,filename,content_id) {
  if (confirm("ยืนยันการลบข้อมูลการร้องขอเอกสารของ "+filename)) {
	ajax_loadContent(span_id,'del_req_nobox.php',id_member,'member',content_id);
	window.location.reload(true);
	/*window.location.replace("members_nodoc.php?iRegister=1");*/
  }
}
